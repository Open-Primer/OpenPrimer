...
🚨 PREVIOUS CRITIQUE:
"The `year` property is present in the `props` of Image components, which is forbidden by the critical media rules. Specifically, `interactiveComponents[1].props.year` and `interactiveComponents[3].props.year` must be removed.
Detailed errors:
- Field "interactiveComponents[1].props.year": The `year` property is forbidden in the `props` of Image components. Please remove it.
- Field "interactiveComponents[3].props.year": The `year` property is forbidden in the `props` of Image components. Please remove it."
