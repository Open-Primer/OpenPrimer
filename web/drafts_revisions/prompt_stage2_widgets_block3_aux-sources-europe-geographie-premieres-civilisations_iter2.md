...
🚨 PREVIOUS CRITIQUE:
"The `Image` component contains a forbidden `year` property. This field is not allowed for Image components as per critical media rules. Additionally, the content of the `year` field is excessively verbose and does not represent a year, indicating a misuse of the property.
Detailed errors:
- Field "interactiveComponents": The `Image` component with `id: carte_grece_antique` contains a `year` property, which is explicitly forbidden for Image components. This property should be removed. Furthermore, the content provided in the `year` field is a lengthy description, not a year, which is an incorrect use of the field."
