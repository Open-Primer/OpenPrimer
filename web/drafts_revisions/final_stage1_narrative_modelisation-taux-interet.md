...
Le prix d'une obligation zéro-coupon $P(t, T)$ à l'instant $t$ qui paie 1 unité monétaire à l'échéance $T$ sous le modèle de Vasicek est donné par $P(t, T) = A(t, T)e^{-B(t, T)r_t}$, où $A(t, T)$ et $B(t, T)$ sont des fonctions déterministes de $t$ et $T$. Pour une dérivation détaillée, on peut se référer à Shreve [[WIDGET:Reference:2]] ou Brigo &amp; Mercurio .
