...
🚨 PREVIOUS CRITIQUE:
"The block contains `Image` components that include the forbidden `year` property in their `props`. According to Critical Media Rules, `Image` components MUST NOT contain `year`.
Detailed errors:
- Field "interactiveComponents[1].props.year": The `Image` component with id `bachelier_thesis` includes the forbidden `year` property in its `props`. This property must be removed.
- Field "interactiveComponents[3].props.year": The `Image` component with id `black_scholes_formula` includes the forbidden `year` property in its `props`. This property must be removed."
