...
🚨 PREVIOUS CRITIQUE:
"The widget block contains placeholder values ('N/A') for the 'year' property in several interactive components. As per the guidelines, 'ZERO placeholders, draft markers, bracketed texts, or template values are present.' The 'year' property should either be omitted or set to 'null' if not applicable for ConceptLink and Glossary components, not 'N/A'.
Detailed errors:
- Field "interactiveComponents": The following interactive components contain 'N/A' as a placeholder for the 'year' property, which is not allowed: 'finance_quantitative', 'retour_moyenne', 'taux_court', 'courbe_taux', and 'no_arbitrage_model'. For ConceptLink and Glossary components, the 'year' property should be omitted or explicitly set to 'null' if no year is available or required."
