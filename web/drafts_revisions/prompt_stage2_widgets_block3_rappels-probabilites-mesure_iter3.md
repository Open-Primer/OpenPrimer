...
🚨 PREVIOUS CRITIQUE:
"The block contains multiple Image components with forbidden 'year' properties. According to Critical Media Rule 5, Image components MUST NOT contain 'year' properties. This needs to be removed from all affected Image components.
Detailed errors:
- Field "interactiveComponents": The following Image components within 'interactiveComponents' contain a forbidden 'year' property in their 'props': 'prob_space_elements', 'types_variables_aleatoires', 'conditional_expectation_info', and 'option_pricing_conceptual'. The 'year' property must be removed from these Image components."
