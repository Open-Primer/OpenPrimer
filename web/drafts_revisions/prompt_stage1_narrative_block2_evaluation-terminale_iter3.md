...
```mermaid
graph TD
    A[Système Terrestre] --> B(Atmosphère)
    A --> C(Hydrosphère)
    A --> D(Lithosphère)
    A --> E(Biosphère)

    B -- Rayonnement, Précipitations --> C
    B -- Température, Vents --> D
    B -- Climat, Gaz --> E

    C -- Érosion, Sédimentation --> D
    C -- Cycle de l'eau, Habitats --> E

    D -- Relief, Sols --> C
    D -- Substrat, Topographie --> E

    E -- Transpiration, Cycle Nutriments --> B
    E -- Protection Sols, Modif. Microclimats --> D

    F[Action Anthropique] --> B
    F --> C
    F --> D
    F --> E
...
"The block contains multiple unresolved `[refX]` placeholders throughout the text, which must be either replaced with actual references or removed. Additionally, several mandated pedagogical widget types are missing from the block, specifically: HistoricalAnecdote, Quiz, Mermaid, and SolvedExercise. These must be integrated into the lesson to meet the content requirements."
