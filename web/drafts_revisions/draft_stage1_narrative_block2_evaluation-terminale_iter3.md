...
1.  **Analyse de Cartes :**
    ...
    *   **Cartes climatiques :** Interprétation des classifications climatiques (par exemple, la classification de [[WIDGET:RealPerson:koppen:Wladimir Köppen]]) [ref4], des isohyètes, des isothermes, des zones de pression. Déduction des régimes de vents dominants, des zones d'influence océanique ou continentale.
...
2.  **Analyse de Graphiques et de Données :**
    *   **Séries chronologiques :** Interprétation de l'évolution temporelle de variables climatiques (température, précipitations, concentration de CO2 atmosphérique [ref6]), hydrologiques (débits fluviaux) ou géomorphologiques (taux d'érosion). Identification des tendances, des cycles, des ruptures et des anomalies.
...
3.  **Analyse de Textes Scientifiques :**
    *   Lecture critique d'extraits d'articles scientifiques, de rapports (par exemple, ceux du [[WIDGET:ConceptLink:giec:GIEC]]) ou d'ouvrages de référence [ref1], [ref2].
...
*   **Modèles climatiques :** Application des principes de la circulation atmosphérique générale (cellules de Hadley, Ferrel, polaire), de l'effet de serre, des bilans énergétiques et hydriques pour comprendre la distribution des climats et leurs variations [ref1], [ref4].
*   **Modèles géomorphologiques :** Utilisation des concepts de l'équilibre dynamique des versants, des cycles d'érosion (par exemple, le cycle de l'érosion de [[WIDGET:RealPerson:davis:William Morris Davis]]), des processus fluviaux, glaciaires, éoliens ou littoraux pour interpréter la formation et l'évolution des paysages [ref3], [ref5].
...
```mermaid
graph TD
    A[Observation & Collecte de Données] --> B(Analyse Critique des Documents)
    B --> C{Identification des Facteurs Clés}
    C --> D[Mobilisation des Connaissances Théoriques]
    D --> E(Application des Modèles)
    E --> F[Synthèse & Interprétation]
    F --> G(Formulation d'une Problématique)
    G --> H[Construction d'une Argumentation Structurée]
    H --> I(Résolution du Problème / Explication du Phénomène)
...
*   **Fondée sur des preuves :** Chaque affirmation doit être justifiée par des éléments concrets tirés des documents analysés (données chiffrées, observations cartographiques, extraits textuels) ou par des références aux modèles théoriques et aux connaissances acquises [ref2].
...
*   **Référencée :** Intégrer les concepts et les théories des auteurs clés du domaine, tels que [[WIDGET:RealPerson:tricart:Jean Tricart]] pour la géomorphologie [ref3] ou [[WIDGET:RealPerson:strahler:Arthur N. Strahler]] pour la géographie physique [ref2].
