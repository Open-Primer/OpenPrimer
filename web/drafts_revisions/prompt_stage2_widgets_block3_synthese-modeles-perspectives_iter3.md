...
🚨 PREVIOUS CRITIQUE:
"The widget block contains Image and Video components that include the forbidden year property in their props. This violates the critical media rules. The year property must be removed from all Image and Video components.
Detailed errors:
- Field "interactiveComponents": Several Image and Video components within the interactiveComponents array contain the year property (e.g., is_lm_diagram, dsge_intro, model_evolution_diagram, future_research_areas, macro_future_challenges, evolution_macro_thought). According to critical media rules, Image and Video components must not contain the year property. Please remove this property from all affected components."
