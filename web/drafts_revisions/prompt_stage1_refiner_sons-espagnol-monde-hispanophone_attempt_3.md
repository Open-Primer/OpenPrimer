...
"The narrative text contains several critical violations related to visual assets formatting and placement:

1.  **Image Caption Placement**: All image captions are placed *above* their respective images or `CustomFigure` components. The policy explicitly requires captions to be placed "directly below" the image.
2.  **`CustomFigure` Component Misuse**: The `CustomFigure` component is consistently misused. Its `caption` attribute is repeatedly used to caption the *subsequent* markdown image (`![alt](src)`) rather than the image specified in its own `src` attribute. This results in a misalignment between images and their captions. Each `CustomFigure` should encapsulate its own image and its corresponding caption.
3.  **Image Numbering Inconsistency**: There is a numbering error in the image sequence. Specifically, the caption for "Figure 2" is followed by a `CustomFigure` whose `caption` attribute refers to "Figure 3", creating confusion and mislabeling of the visual elements.
4.  **Alt Text for Factual Images**: For the image corresponding to "Figure 2" (the sagittal section of the human head, `https://upload.wikimedia.org/wikipedia/commons/thumb/e/e1/Sagittal_section_of_the_human_head_with_phonetic_annotations.svg/...`), the `alt` text `Vocal_tract_diagram` is not an English Wikipedia page title. It must be corrected to `Sagittal_section_of_the_human_head_with_phonetic_annotations` to comply with the policy for factual images.

To rectify these issues, please ensure:
*   All captions are moved to appear immediately *below* their corresponding images or `CustomFigure` components.
*   Each `CustomFigure` component correctly encapsulates its own image (`src` attribute) and its corresponding caption (`caption` attribute).
*   Image numbering is sequential and consistent throughout the lesson.
*   The `alt` text for the sagittal section image is updated to `Sagittal_section_of_the_human_head_with_phonetic_annotations`."
...
*Figure 1: Schéma des voyelles espagnoles dans le trapèze vocalique - Position de la langue et ouverture de la bouche pour chaque son. Ce diagramme illustre la distribution équilibrée des cinq voyelles cardinales de l'espagnol, soulignant leur nature tendue et leur absence de variation contextuelle majeure. Source: Wikimedia Commons*
![Spanish_vowel_chart](https://upload.wikimedia.org/wikipedia/commons/thumb/d/d4/Spanish_vowel_chart.svg/800px-Spanish_vowel_chart.png)
...
*Figure 2: Diagramme sagittal du tractus vocal humain - Illustration des principaux articulateurs (lèvres, dents, alvéoles, palais dur, palais mou, luette, langue, pharynx, glotte) impliqués dans la production des sons de la parole. Source: Wikimedia Commons*
<CustomFigure src="https://upload.wikimedia.org/wikipedia/commons/thumb/e/e1/Sagittal_section_of_the_human_head_with_phonetic_annotations.svg/800px-Sagittal_section_of_the_human_head_with_phonetic_annotations.png" alt="Vocal_tract_diagram" caption="Figure 3: Comparaison des points d'articulation dentale et alvéolaire - Ce schéma illustre la différence entre une articulation dentale (langue contre les dents, comme le /d/ espagnol) et une articulation alvéolaire (langue contre la crête alvéolaire, comme le /d/ français). Source: Adapté de sources linguistiques, AI-generated" fallbackText="" fallbackUrl="" />
![Dental_vs_Alveolar_Articulation](https://image.pollinations.ai/prompt/diagram_comparing_dental_and_alveolar_articulation_for_consonants_showing_tongue_position_relative_to_teeth_and_alveolar_ridge_linguistics_diagram_academic_style?width=640&amp;height=480&amp;nologo=true&amp;private=true)
...
1.  **Mots *Agudas*** (oxytons) : L'accent tonique tombe sur la **dernière syllabe**.
    ...
    *   **Exemples sans tilde** : `ciudád` (ville), `papél` (papier), `reloj` (horloge).
...
*Figure 7: Une personne pratiquant la prononciation devant un microphone - L'importance de l'écoute, de l'enregistrement et de l'auto-correction pour l'amélioration continue de la production phonétique. Source: AI-generated*
<CustomFigure src="https://image.pollinations.ai/prompt/person_practicing_pronunciation_in_front_of_a_microphone_focused_linguistics_student_academic_setting_with_sound_waves_visualized?width=640&amp;amp%3Bheight=480&amp;amp%3Bnologo=true&amp;amp%3Bprivate=true&amp;height=480&amp;nologo=true&amp;private=true" alt="Person_practicing_pronunciation" caption="Figure 8: Représentation conceptuelle de l'ingénierie linguistique pour l'apprentissage des langues - Une illustration abstraite de la construction et de l'optimisation des compétences linguistiques, symbolisant la précision et la méthode. Source: AI-generated" fallbackText="" fallbackUrl="" />
![Language_Engineering_Concept](https://image.pollinations.ai/prompt/conceptual_image_of_language_engineering_with_gears_circuits_and_sound_waves_representing_precision_and_methodical_language_acquisition_academic_style?width=640&amp;height=480&amp;nologo=true&amp;private=true)
...
<References itemsBase64="" />
