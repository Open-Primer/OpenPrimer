...
🚨 PREVIOUS CRITIQUE:
"The block contains Image components with forbidden properties. Specifically, the 'year' property is present in multiple Image components, which violates the critical media rules.
Detailed errors:
- Field "interactiveComponents": The following Image components contain the forbidden 'year' property in their 'props': 'delta_call_graph', 'gamma_option_graph', 'vega_option_graph', and 'hedging_challenges'. The 'year' property is not allowed for Image components and must be removed."
