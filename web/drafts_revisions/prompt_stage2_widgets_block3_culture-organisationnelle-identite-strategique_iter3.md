...
🚨 PREVIOUS CRITIQUE:
"The block contains multiple Image components that include the forbidden 'year' property within their 'props'. According to Critical Media Rules, Image components MUST NOT contain 'year'. This property must be removed from all Image components.
Detailed errors:
- Field "interactiveComponents": Several Image components within this array contain the forbidden 'year' property in their 'props'. Specifically, 'culture_strategie_interdependance', 'schein_iceberg', 'transmission_culture', and 'culture_strategy_alignment' all include 'year'. Please remove the 'year' property from the 'props' of all Image components."
