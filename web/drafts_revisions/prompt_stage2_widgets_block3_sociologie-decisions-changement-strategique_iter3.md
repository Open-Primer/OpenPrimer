...
🚨 PREVIOUS CRITIQUE:
"The widget block contains media components that violate critical media rules. Specifically, an Image component and a Video component include forbidden properties in their 'props' object.
Detailed errors:
- Field "interactiveComponents": The 'interactiveComponents' array contains two components with invalid properties. The Image component with 'id: decision_influences' includes a 'year' property in its 'props', which is forbidden for Image components. Similarly, the Video component with 'id: gestion_resistance' also includes a 'year' property in its 'props', which is forbidden for Video components. These properties are resolved automatically downstream and should not be present in the raw widget JSON."
