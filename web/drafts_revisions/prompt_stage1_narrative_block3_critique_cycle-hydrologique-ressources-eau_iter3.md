...
```mermaid
graph TD
    A[Cycle Hydrologique] --> B(Modélisation Avancée & Prévision)
    A --> C(Télédétection & SIG)
    A --> D(Écohydrologie & SFN)
    A --> E(Socio-hydrologie & Gouvernance)
    A --> F(Nexus Eau-Énergie-Alimentation)
    A --> G(Qualité de l'eau & Contaminants Émergents)

    B --> H{Décisions de Gestion de l'Eau}
    C --> H
    D --> H
    E --> H
    F --> H
    ...
    H --> I[Gestion Durable de l'Eau]
    I --> J(Résilience Hydrique)
    I --> K(Sécurité Alimentaire & Énergétique)
    I --> L(Préservation des Écosystèmes)
```
