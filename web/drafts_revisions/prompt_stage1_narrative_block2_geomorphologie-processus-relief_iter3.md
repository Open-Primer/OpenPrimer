...
* Heading: "## Les Processus Exogènes et l'Évolution des Paysages"
  Instructions: "Détailler les forces externes qui modèlent le relief. Inclure l'altération (physique, chimique, biologique), l'érosion (fluviale, glaciaire, éolienne, marine) et le transport des sédiments. Expliquer comment ces processus interagissent pour sculpter les formes terrestres."
* Heading: "## Les Principales Formes du Relief Terrestre"
  Instructions: "Présenter les caractéristiques et la genèse des grandes formes de relief. Inclure les montagnes (chaînes de montagnes, massifs), les plaines et plateaux, les littoraux (plages, falaises, deltas) et les déserts (dunes, regs). Illustrer avec des exemples concrets et discuter de leur dynamique évolutive."
...
- Lesson Title: "Géomorphologie: processus et formes du relief terrestre"
...
"The lesson block is well-structured and informative, but it contains unresolved placeholders and critically fails to meet the pedagogical requirements for interactive elements. Specifically: 1. All `[refX]` placeholders must be replaced with actual references or removed. 2. The lesson currently only includes 2 inline hover-cards (`Glossary` and `ConceptLink`), falling short of the minimum requirement of 3. At least one more inline widget should be added. 3. The lesson is missing several mandated block widget types: `Quiz`, `SolvedExercise`, and `UnsolvedExercise`. These are essential for active learning and assessment and must be integrated into the lesson to ensure pedagogical completeness."
