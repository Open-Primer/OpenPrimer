...
🚨 PREVIOUS CRITIQUE:
"The 'interactiveComponents' array contains duplicate 'id' values, which must be unique for each component. Specifically, 'arbitrage' and 'black_scholes' are used more than once.
Detailed errors:
- Field "interactiveComponents": The 'id' field for each component within the 'interactiveComponents' array must be unique. The 'id' 'arbitrage' is duplicated (used for a ConceptLink and a Glossary component). The 'id' 'black_scholes' is also duplicated (used for a RealPerson and a ConceptLink component). Please ensure all 'id's are unique."
