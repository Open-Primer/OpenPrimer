...
🚨 PREVIOUS CRITIQUE:
"Some Image components within 'interactiveComponents' contain forbidden properties. Specifically, the 'year' property is present in the 'objectifs_politique_monetaire' and 'taylor_rule_historical_fit' Image components, which is not allowed according to Critical Media Rules.
Detailed errors:
- Field "interactiveComponents": The Image component with id 'objectifs_politique_monetaire' contains the forbidden 'year' property in its 'props'. The Image component with id 'taylor_rule_historical_fit' also contains the forbidden 'year' property in its 'props'. Image components must not contain 'year', 'url', 'wikipediaUrl', 'wikipediaLink', or 'imageUrl' properties."
