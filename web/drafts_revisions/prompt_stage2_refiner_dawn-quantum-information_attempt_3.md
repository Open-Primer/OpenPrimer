...
1.  **Perfect Semantic & Anchor Alignment**: The `interactiveComponents` array is incomplete. Several `[[WIDGET:id]]` anchors present in the narrative draft do not have corresponding entries in the `interactiveComponents` array. Specifically, the following widget types and their respective IDs are missing: `DidYouKnow:ultraviolet_catastrophe`, `CustomFigure:max_planck_1918`, `CustomFigure:photoelectric_effect_diagram`, `PointOfView:wave_function_interpretation`, `CustomFigure:electron_diffraction_pattern`, `Epistemology:quantum_reality_debate`, `CustomFigure:bell_test_experiment`, `CriticalThinking:bell_theorem_implications`, `Mermaid:quantum_timeline`, `CustomFigure:qubit_representation`, and `Image:quantum_information_conceptual_diagram`. All interactive anchors must be explicitly declared in `interactiveComponents`.
2.  **MCQ and Diagnostic Correctness & Flat-Prop Format**: The `finalEvaluation` quiz and the `foundational_concepts` quiz (within `interactiveComponents`) do not adhere to the specified 'Flat-Prop Quiz Format'. For both quizzes, the question objects use the key `q` instead of `question`, and the `options` are structured as an array of objects with `text` and `correct` properties. The required format is an array of questions with a `question` string, an array of `options` strings, and a separate `correctIndex` (0-indexed integer) for the correct answer, along with an `explanation` string. Please update the structure of these quizzes to match the 'Flat-Prop Quiz Format'.
3.  **Academic Bibliography & Citation Style**: The `references` array contains 6 entries, which exceeds the specified limit of '3 to 5 real, authoritative academic works'. Please reduce the number of references to be within this range."
...
---
{
  ...
  "diagnosticQuiz": {
    ...
    "correctIndex": 2,
    "targetSectionId": "the-classical-worldview-and-its-unraveling-seeds-of-discontent",
    "sectionTitle": "1. The Classical Worldview and Its Unraveling: Seeds of Discontent"
  },
  "learningObjectives": {
    "knowledge": [
      ...
      "Analyze the concept of quantum entanglement and its implications for local realism.",
      "Evaluate the transition from quantum paradoxes to the theoretical foundations of quantum information and computation."
    ],
    "skills": [
