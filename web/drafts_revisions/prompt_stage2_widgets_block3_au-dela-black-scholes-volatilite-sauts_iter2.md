...
🚨 PREVIOUS CRITIQUE:
"The widget block contains multiple image components that violate the CRITICAL MEDIA RULES. Specifically, five image components include the forbidden 'year' property within their 'props'. Image components MUST NOT contain 'year', 'url', 'wikipediaUrl', 'wikipediaLink', or 'imageUrl' properties. These properties are automatically resolved downstream and their presence in the raw widget block is an error. Please remove the 'year' property from all affected image components.
Detailed errors:
- Field "interactiveComponents": The 'interactiveComponents' array contains several image components with forbidden properties. The components with IDs 'bsm_assumptions_vs_reality', 'volatility_smile_example', 'heston_sdes', 'jump_diffusion_path', and 'model_complexity_tradeoff' all incorrectly include a 'year' property in their 'props'. This violates the CRITICAL MEDIA RULES for Image components. Remove the 'year' property from the 'props' of these five image components."
