...
```mermaid
graph TD
    A[Source Lumineuse (ex: laser)] --> B(Fente Unique S0)
    B -- Lumière cohérente --> C1(Fente S1)
    B -- Lumière cohérente --> C2(Fente S2)
    C1 -- Onde sphérique --> D[Écran d'Observation]
    C2 -- Onde sphérique --> D
    D --> E{Motif d'Interférence: Franges Lumineuses et Sombres}
    subgraph Expérience des Fentes de Young
        A
        B
        C1
        C2
...
*Schéma simplifié de l'expérience des fentes de Young, illustrant la propagation de la lumière à travers deux fentes et la formation d'un motif d'interférence sur l'écran d'observation. Ce phénomène est
