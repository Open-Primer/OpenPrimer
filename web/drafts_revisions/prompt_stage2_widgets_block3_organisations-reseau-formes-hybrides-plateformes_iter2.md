...
🚨 PREVIOUS CRITIQUE:
"The widget block contains several critical media rule violations. Specifically, Image components include the forbidden 'year' property within their 'props', and Video components include the forbidden 'id' property at their top level. These properties must be removed as they are automatically resolved downstream.
Detailed errors:
- Field "interactiveComponents": Several components within the 'interactiveComponents' array violate critical media rules. Image components (ids: 'reseau_organisationnel', 'continuum_organisationnel', 'modele_plateforme_numerique', 'defis_opportunites_management') incorrectly include the 'year' property in their 'props'. Video components (ids: 'reseaux_organisationnels_explication', 'impacts_plateformes_societe', 'future_of_organizations') incorrectly include the 'id' property at their top level. These forbidden properties must be removed from the respective component definitions."
