...
🚨 PREVIOUS CRITIQUE:
"The 'year' property for 'ConceptLink' and 'Glossary' components contains placeholder-like string values instead of being 'null' (the JSON keyword) or omitted. This violates the requirement for zero placeholders and incomplete fields.
Detailed errors:
- Field "interactiveComponents": The 'year' property in the 'props' object for 'ConceptLink' components ('politique_economique', 'anticipations_rationnelles') and 'Glossary' components ('modelisation_macroeconomique') is set to a string value like 'null (concept)' or 'null (glossary term)'. According to the guidelines, this property should either be 'null' (the JSON keyword) or entirely omitted if not applicable, not a descriptive string placeholder."
