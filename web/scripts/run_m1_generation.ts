...
async function main() {
  ...
  // ── Cours cible : Intelligence Artificielle & Apprentissage Automatique (M1) ──
  // Sujet riche en médias : images d'architectures NN, vidéos de démos, extraits audio
  const targetCourseName = "Intelligence Artificielle et Apprentissage Automatique";
  ...
  try {
    ...
    if (fs.existsSync(draftsDir)) {
      const draftFiles = fs.readdirSync(draftsDir)
        .filter(f => f.includes(targetSlug) || f.includes('intelligence') || f.includes('apprentissage'))
        .sort();
