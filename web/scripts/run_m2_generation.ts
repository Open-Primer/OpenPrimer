...
async function main() {
  ...
  // ── Cours cible : Deep Learning Avancé et Modèles Génératifs (M2) ──
  const targetCourseName = "Deep Learning Avancé et Modèles Génératifs";
  const targetLevel = "M2";
  const targetLang = "fr";
  ...
  console.log(`   🎓 Niveau  : "${targetLevel}" (Master 2ème année)`);
  ...
  try {
    ...
    if (fs.existsSync(draftsDir)) {
      const draftFiles = fs.readdirSync(draftsDir)
        .filter(f => f.includes(targetSlug) || f.includes('deep') || f.includes('learning') || f.includes('generatifs'))
        .sort();
