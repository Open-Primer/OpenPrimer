...
async function main() {
  ...
  // ── Cours cible : Acoustique Physique et Acoustique Musicale (L2) ──
  const targetCourseName = "Acoustique Physique et Acoustique Musicale";
  const targetLevel = "L2";
  const targetLang = "fr";
  ...
  console.log(`\n${'='.repeat(60)}`);
  console.log(`🗑  Nettoyage des enregistrements existants pour "${targetSlug}"...`);
  ...
  console.log(`   🎓 Niveau  : "${targetLevel}" (Licence 2ème année)`);
  ...
  try {
    ...
    if (fs.existsSync(draftsDir)) {
      const draftFiles = fs.readdirSync(draftsDir)
        .filter(f => f.includes(targetSlug) || f.includes('acoustique') || f.includes('physique') || f.includes('musicale'))
        .sort();
