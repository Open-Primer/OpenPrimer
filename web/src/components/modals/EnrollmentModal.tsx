  ...
  return (
    ...
    >
      <motion.div
        ...
        className="max-w-2xl w-full bg-slate-900 border border-slate-800 rounded-[40px] p-8 md:p-10 shadow-2xl relative max-h-[85vh] overflow-y-auto custom-scrollbar cursor-default"
      >
        ...
        {history.length > 0 && (
          <button
            onClick={handleBack}
            className="mb-6 flex items-center gap-2 px-3 py-1.5 rounded-xl bg-slate-950 hover:bg-slate-900 border border-slate-800 text-[10px] font-black uppercase tracking-wider text-slate-400 hover:text-white transition-all cursor-pointer"
          >
              ...
              {fetchedChildCourses.map((child: any) => {
                ...
                return (
                  <div
                    ...
                    className="p-4 bg-slate-950/50 hover:bg-slate-900/80 border border-slate-800 hover:border-blue-500/30 hover:scale-[1.02] active:scale-[0.98] transition-all rounded-2xl cursor-pointer flex flex-col justify-between group"
                  >
                    <div>
                      <div className="flex justify-between items-center mb-2">
                        <span className="text-[8px] font-black uppercase tracking-wider px-2 py-0.5 bg-slate-800 text-slate-400 rounded-md">
                          {child.level}
                        </span>
        ...
        {prerequisites.length > 0 && (
          <div className="mb-8 p-5 bg-slate-950/30 border border-slate-800 rounded-2xl">
            <p className="text-[9px] font-black uppercase text-slate-500 tracking-wider mb-3 text-left">
              ...
              {prerequisites.map((pre: string, idx: number) => {
                ...
                return (
                  <div 
                    ...
                    className={`flex items-center justify-between p-3 bg-slate-950/50 rounded-xl border border-slate-800/60 transition-all ${
                      matchedCourse 
        ...
        {!activeCourse.isCurriculum && (
          <div className="space-y-6 mb-10 text-left">
            <p className="text-[9px] font-black uppercase text-slate-500 tracking-widest border-b border-slate-800 pb-2">{t.syllabus_overview || "Syllabus Overview"}</p>
              ...
              <div key={uIdx} className="space-y-3">
                ...
                <div className="grid gap-2 pl-6">
                  {(unit.modules || []).map((mod: string, mIdx: number) => (
                    <div key={mIdx} className="px-4 py-2 bg-slate-950/20 border border-slate-800 rounded-xl text-xs text-slate-300">
                      {mod}
                    </div>
                  ))}
                </div>
