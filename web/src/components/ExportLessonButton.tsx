...
export const ExportLessonButton = ({ title, subject, level, content, lang, courseSlug, version }: ExportLessonButtonProps) => {
  ...
  const handleExportRevisionSheet = () => {
    ...
    if (glossaryTerms.length > 0) {
      mermaidMindmap += `    ${lang === 'fr' ? 'Notions' : 'Concepts'}\n`;
    ...
    if (summaryItems.length > 0) {
      mermaidMindmap += `    ${lang === 'fr' ? 'Points Clés' : 'Key Points'}\n`;
...
`;
    ...
    if (summaryItems.length > 0) {
      revisionContent += `\n## 📝 ${lang === 'fr' ? 'Résumé & Points Clés' : 'Summary & Key Points'}\n`;
    ...
    if (quizQuestions.length > 0) {
      revisionContent += `\n## 🧠 ${lang === 'fr' ? 'Questions d\'Auto-évaluation' : 'Self-Assessment Questions'}\n`;
...
<head>
  <meta charset="UTF-8">
  <title>${lang === 'fr' ? 'Fiche de Révision' : 'Revision Sheet'} — ${title}</title>
  <style>
    ${getPremiumStyles()}
    @media print {
    ...
    .badge-ai { display: inline-block; padding: 3px 10px; font-size: 0.7rem; font-weight: 700;
      background: rgba(139,92,246,0.15); color: #a78bfa; border: 1px solid rgba(139,92,246,0.3);
      border-radius: 9999px; letter-spacing: 0.08em; text-transform: uppercase; }
    .key-point { padding: 14px 20px; border-left: 3px solid #4f46e5;
      background: rgba(79,70,229,0.06); border-radius: 0 12px 12px 0; margin-bottom: 12px;
      font-size: 0.95rem; }
    .glossary-entry { padding: 10px 0; border-bottom: 1px solid var(--border); }
    .glossary-entry strong { color: var(--text-main); }
    .quiz-q { padding: 10px 0; border-bottom: 1px solid var(--border); font-size: 0.9rem; color: var(--text-muted); }
    .print-btn { display: inline-flex; align-items: center; gap: 8px; margin-top: 24px;
      padding: 10px 24px; background: #4f46e5; color: white; font-weight: 700;
      font-size: 0.85rem; border: none; border-radius: 12px; cursor: pointer;
...
<body>
  <header>
    ...
    <h1>${lang === 'fr' ? 'Fiche de Révision' : 'Revision Sheet'}</h1>
    ...
    <button class="print-btn no-print" onclick="window.print()">
      🖨️ ${lang === 'fr' ? 'Imprimer / Exporter PDF' : 'Print / Export PDF'}
    </button>
  </header>

  <main>
    ${summaryItems.length > 0 ? `
    <h2>📝 ${lang === 'fr' ? 'Résumé & Points Clés' : 'Summary & Key Points'}</h2>
    ${summaryItems.map(item => `<div class="key-point">${item}</div>`).join('')}
    ` : ''}

    <h2>📚 ${lang === 'fr' ? 'Glossaire des Notions Clés' : 'Key Concepts Glossary'}</h2>
    ${glossaryTerms.length > 0
      ? glossaryTerms.map(item =>
          `<div class="glossary-entry"><strong>${item.term}</strong> : ${item.definition}</div>`
        ).join('')
      : `<p><em>${lang === 'fr' ? 'Aucune notion glossaire définie.' : 'No glossary definitions found.'}</em></p>`
    }

    ${quizQuestions.length > 0 ? `
    <h2>🧠 ${lang === 'fr' ? 'Questions d\'Auto-évaluation' : 'Self-Assessment Questions'}</h2>
    ${quizQuestions.slice(0, 12).map((q, idx) =>
      `<div class="quiz-q">${idx + 1}. ${q}</div>`
    ).join('')}
    ` : ''}
  </main>
</body>
                  ...
                  <div className="flex-1 space-y-2">
                    ...
                    <div className="flex flex-wrap gap-2 pt-2">
                      ...
                      >
                        ...
                        <span>{lang === 'fr' ? 'Fiche HTML/PDF' : 'Sheet HTML/PDF'}</span>
                      </button>
                    </div>
                  </div>
                </div>
