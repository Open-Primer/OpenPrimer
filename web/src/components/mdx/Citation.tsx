...
export const Citation = ({
  ...
  return (
    ...
    {/* Quote Main Content */}
    <div className="text-[16px] leading-relaxed text-slate-850 dark:text-slate-250 pl-1 italic font-medium my-2">
      {quoteText}
    </div>

    {/* Author & Source Attribution */}
    {(author || source || year) && (
      <div className="mt-3 pl-1 flex flex-wrap items-center gap-x-2 gap-y-1 text-[13px] text-slate-550 dark:text-slate-450">
        ...
        {original && (
          <button
            ...
            className="ml-auto flex items-center gap-1 text-[12px] text-indigo-500 hover:text-indigo-600 dark:text-indigo-400 dark:hover:text-indigo-300 transition-colors font-bold cursor-pointer select-none border-none bg-transparent p-0"
          >
    ...
    {original && showOriginal && (
      <div className="mt-3 p-3 bg-slate-100/50 dark:bg-slate-900/50 border border-dashed border-slate-200 dark:border-slate-800 rounded-xl transition-all duration-300">
        <div className="text-[11px] font-black uppercase tracking-widest text-slate-400 dark:text-slate-500 mb-1 select-none">
          {normalizedLang === 'fr' ? 'Version Originale' : 'Original Version'}
        </div>
        <div className="text-[15px] italic text-slate-600 dark:text-slate-400 leading-relaxed pl-1">
          {original}
        </div>
      </div>
    )}
    ...
    {/* Commentary */}
    {commentary && (
      <div className="mt-4 text-[15px] leading-relaxed text-slate-700 dark:text-slate-350 select-text">
        {commentary}
      </div>
    )}
    </div>
