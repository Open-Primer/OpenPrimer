...
const MAP_STRINGS = {
  EN: {
    title: "Interactive Climate Impact Map",
    subtitle: "Projected regional impacts of climate change on natural and human systems",
    selectPrompt: "Click on a region to view detailed climate projections",
    water: "Water Resources",
    agriculture: "Agriculture & Food",
    biodiversity: "Biodiversity & Ecosystems",
    extremes: "Extreme Events",
    severity: "Risk Level",
    viewImage: "View Institutional Source Map",
    viewInteractive: "View Interactive Dashboard",
    regions: {
      na: "North America",
      sa: "South America",
      eu: "Europe",
  ...
  FR: {
    title: "Carte d'impact climatique interactive",
    subtitle: "Impacts régionaux projetés du changement climatique sur les systèmes naturels et humains",
    selectPrompt: "Cliquez sur une région pour voir les projections climatiques détaillées",
    water: "Ressources en eau",
    agriculture: "Agriculture & Alimentation",
    biodiversity: "Biodiversité & Écosystèmes",
    extremes: "Événements extrêmes",
    severity: "Niveau de risque",
    viewImage: "Voir la carte source institutionnelle",
    viewInteractive: "Voir le tableau de bord interactif",
    regions: {
      na: "Amérique du Nord",
      sa: "Amérique du Sud",
      eu: "Europe",
...
const ClimateImpactMap = ({ 
  ...
  // Enforce "No Fallback to AI" policy: if the image URL is pollinations.ai, we treat it as invalid/blocked.
  const isBlocked = src && src.includes('pollinations.ai');

  // Let's decide if a valid institutional image is available
  const hasValidImage = src && !isBlocked && !failed && !unresolved;

  React.useEffect(() => {
    // Register figure for counting/numbering
    registerFigure(id);
    return () => {
      unregisterFigure(id);
    };
  }, [id, registerFigure, unregisterFigure]);

  const langKey = ((language || 'EN').toUpperCase() === 'FR' ? 'FR' : 'EN') as 'EN' | 'FR';
  const t = MAP_STRINGS[langKey] || MAP_STRINGS.EN;
  ...
  const [selectedRegion, setSelectedRegion] = React.useState<string>('eu');

  // Regions data
  const regionData: Record<string, {
    name: string;
    water: { risk: 'moderate' | 'high' | 'severe' | 'critical'; desc: string };
    agriculture: { risk: 'moderate' | 'high' | 'severe' | 'critical'; desc: string };
    biodiversity: { risk: 'moderate' | 'high' | 'severe' | 'critical'; desc: string };
    extremes: { risk: 'moderate' | 'high' | 'severe' | 'critical'; desc: string };
  }> = React.useMemo(() => {
    ...
    return {
      eu: {
        ...
        water: {
          risk: 'severe',
          desc: isFR 
            ? "Risque de sécheresse accru dans le Sud, inondations fluviales fréquentes au Nord et à l'Est."
            : "Increased drought risk in Southern Europe, frequent river flooding in Northern and Eastern regions."
        },
        agriculture: {
          risk: 'high',
          desc: isFR
            ? "Baisse des rendements des cultures méridionales ; gains potentiels temporaires en Europe du Nord."
            : "Decline in Southern crop yields; temporary potential gains in Northern Europe."
        },
        biodiversity: {
          risk: 'high',
          desc: isFR
            ? "Migration des espèces vers le nord et en altitude ; dégradation des écosystèmes alpins."
            : "Northward and upward species migration; degradation of alpine and montane ecosystems."
        },
        extremes: {
          risk: 'critical',
          desc: isFR
            ? "Canicules intenses de plus en plus fréquentes, feux de forêt dévastateurs en zone méditerranéenne."
            : "More frequent and intense heatwaves, devastating wildfire risks in the Mediterranean basin."
        }
      },
      na: {
        name: t.regions.na,
        water: {
          risk: 'severe',
          desc: isFR
            ? "Diminution importante du manteau neigeux de l'Ouest, pénuries d'eau critiques dans le Sud-Ouest."
            : "Significant western snowpack decline, critical water shortages in the Southwest."
        },
        agriculture: {
          risk: 'high',
          desc: isFR
            ? "Déplacement des ceintures agricoles vers le nord, stress thermique important sur le bétail."
            : "Northward shift of agricultural belts, high heat stress on livestock."
        },
        biodiversity: {
          risk: 'severe',
          desc: isFR
            ? "Mortalité forestière accrue (parasites, incendies), perturbation des zones humides côtières."
            : "Increased forest mortality from pests and wildfires, disruption of coastal wetlands."
        },
        extremes: {
          risk: 'critical',
          desc: isFR
            ? "Ouragans de forte intensité accrus, mégafeux à l'Ouest, vagues de chaleur extrêmes."
            : "Increased high-intensity hurricanes, western megafires, and extreme heatwaves."
        }
      },
      sa: {
        name: t.regions.sa,
        water: {
          risk: 'critical',
          desc: isFR
            ? "Retrait accéléré des glaciers andins affectant l'approvisionnement en eau potable de millions de personnes."
            : "Accelerated retreat of Andean glaciers affecting drinking water supply for millions."
        },
        agriculture: {
          risk: 'severe',
          desc: isFR
            ? "Chute des rendements du soja et du maïs, vulnérabilité extrême de l'agriculture de subsistance."
            : "Declines in soybean and corn yields, extreme vulnerability of subsistence agriculture."
        },
        biodiversity: {
          risk: 'critical',
          desc: isFR
            ? "Risque élevé de savannisation de la forêt amazonienne et perte massive de biodiversité unique."
            : "High risk of Amazon forest savannization and massive loss of unique biodiversity."
        },
        extremes: {
          risk: 'severe',
          desc: isFR
            ? "Sécheresses historiques en Amazonie, inondations soudaines et glissements de terrain dévastateurs."
            : "Historical droughts in the Amazon, devastating flash floods and landslides."
        }
      },
      af: {
        name: t.regions.af,
        water: {
          risk: 'critical',
          desc: isFR
            ? "Stress hydrique extrême affectant plus de la moitié de la population ; baisse des nappes phréatiques."
            : "Extreme water stress affecting over half of the population; declining groundwater tables."
        },
        agriculture: {
          risk: 'critical',
          desc: isFR
            ? "Réduction jusqu'à 50% des rendements agricoles pluviaux, insécurité alimentaire généralisée."
            : "Up to 50% reduction in rain-fed crop yields, widespread food insecurity."
        },
        biodiversity: {
          risk: 'severe',
          desc: isFR
            ? "Extinction de masse d'espèces endémiques (Madagascar, Fynbos), dégradation des savanes."
            : "Mass extinction of endemic species (Madagascar, Fynbos), savanna degradation."
        },
        extremes: {
          risk: 'critical',
          desc: isFR
            ? "Désertification accélérée, sécheresses prolongées au Sahel et inondations côtières en Afrique de l'Ouest."
            : "Accelerated desertification, prolonged Sahel droughts, and West African coastal flooding."
        }
      },
      as: {
        name: t.regions.as,
        water: {
          risk: 'critical',
          desc: isFR
            ? "Fonte accélérée des glaciers de l'Himalaya perturbant le débit des grands fleuves nourriciers."
            : "Accelerated melting of Himalayan glaciers disrupting major river basins and water supply."
        },
        agriculture: {
          risk: 'severe',
          desc: isFR
            ? "Baisse drastique de la production de riz en Asie du Sud-Est due à la salinisation des deltas."
            : "Drastic decline in Southeast Asian rice production due to delta salinization."
        },
        biodiversity: {
          risk: 'severe',
          desc: isFR
            ? "Blanchissement massif des récifs coralliens tropicaux, perte des mangroves côtières."
            : "Massive bleaching of tropical coral reefs, loss of coastal mangrove protections."
        },
        extremes: {
          risk: 'critical',
          desc: isFR
            ? "Moussons plus intenses et erratiques, inondations catastrophiques, vagues de chaleur mortelles."
            : "More intense and erratic monsoons, catastrophic flooding, deadly heatwaves."
        }
      },
      oc: {
        name: t.regions.oc,
        water: {
          risk: 'severe',
          desc: isFR
            ? "Pénuries d'eau douce chroniques en Australie méridionale et salinisation des aquifères insulaires."
            : "Chronic freshwater shortages in Southern Australia and salinization of island aquifers."
        },
        agriculture: {
          risk: 'high',
          desc: isFR
            ? "Baisse de la productivité céréalière et de l'élevage en Australie, perturbation des cultures insulaires."
            : "Decrease in grain and livestock productivity in Australia, disruption of island farming."
        },
        biodiversity: {
          risk: 'critical',
          desc: isFR
            ? "Destruction irréversible de la Grande Barrière de Corail, dégradation des forêts d'eucalyptus."
            : "Irreversible destruction of the Great Barrier Reef, degradation of eucalyptus forests."
        },
        extremes: {
          risk: 'critical',
          desc: isFR
            ? "Méga-incendies dévastateurs en Australie, submersion côtière des îles du Pacifique par la hausse de la mer."
            : "Devastating mega-bushfires in Australia, coastal inundation of low-lying Pacific islands."
        }
      },
      po: {
        name: t.regions.po,
        water: {
          risk: 'severe',
          desc: isFR
            ? "Fonte rapide de la calotte glaciaire, acidification de l'océan Arctique et libération d'eau douce."
            : "Rapid ice sheet melting, Arctic ocean acidification, and massive freshwater release."
        },
        agriculture: {
          risk: 'moderate',
          desc: isFR
            ? "Extension potentielle des zones de végétation forestière, mais sols instables (thermokarst)."
            : "Potential northward extension of boreal vegetation, but unstable soils due to permafrost thaw."
        },
        biodiversity: {
          risk: 'critical',
          desc: isFR
            ? "Effondrement des écosystèmes dépendant de la banquise (ours polaires, phoques, manchots)."
            : "Collapse of sea-ice dependent ecosystems (polar bears, seals, penguins)."
        },
        extremes: {
          risk: 'critical',
          desc: isFR
            ? "Réchauffement 3 à 4 fois supérieur à la moyenne mondiale, dégel du pergélisol libérant du méthane."
            : "Warming 3 to 4 times the global average, rapid permafrost thaw releasing methane."
        }
      }
    };
  }, [t, langKey]);
  ...
  return (
    <div 
      id={figureNumber !== null ? `figure-${figureNumber}` : undefined}
      className="my-8 w-full flex flex-col items-center justify-center gap-4 scroll-mt-24"
    >
      {/* View Mode Toggle if resolved image is available */}
      {hasValidImage && (
          ...
          <button
            ...
            className={`px-3 py-1.5 text-xs font-semibold rounded-lg border transition-all ${
              viewMode === 'interactive'
                ...
                : 'bg-slate-100 text-slate-700 border-slate-200 dark:bg-slate-800 dark:text-slate-300 dark:border-slate-700 hover:bg-slate-200 dark:hover:bg-slate-750'
            }`}
          >
            {t.viewInteractive}
          </button>
      ...
      {hasValidImage && viewMode === 'image' ? (
        ...
        <div className="w-full bg-slate-50 dark:bg-slate-900/30 border border-slate-200 dark:border-slate-800/60 rounded-3xl p-4 md:p-6 shadow-md transition-all duration-300">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
            {/* Map Area */}
            <div className="lg:col-span-7 flex flex-col items-center justify-center bg-slate-100/50 dark:bg-slate-950/20 border border-slate-150 dark:border-slate-850/50 rounded-2xl p-4 relative overflow-hidden">
              <svg 
                viewBox="0 0 800 450" 
              ...
              >
                ...
                {/* North America */}
                <polygon 
                  points="100,80 250,80 280,180 200,220 130,170" 
                  className={`transition-colors duration-300 cursor-pointer ${
                    selectedRegion === 'na' 
                      ? 'fill-indigo-500/20 stroke-indigo-500 stroke-2' 
                      : 'fill-slate-300/10 dark:fill-slate-700/10 stroke-slate-400/25 dark:stroke-slate-500/20 hover:fill-indigo-500/10 hover:stroke-indigo-500/50'
                  }`}
                  onClick={() => setSelectedRegion('na')}
                />
                {/* South America */}
                <polygon 
                  points="230,230 310,230 350,330 300,400 240,320" 
                  className={`transition-colors duration-300 cursor-pointer ${
                    selectedRegion === 'sa' 
                      ? 'fill-indigo-500/20 stroke-indigo-500 stroke-2' 
                      : 'fill-slate-300/10 dark:fill-slate-700/10 stroke-slate-400/25 dark:stroke-slate-500/20 hover:fill-indigo-500/10 hover:stroke-indigo-500/50'
                ...
                {/* Europe */}
                <polygon 
                  points="360,80 470,80 480,160 410,180 370,140" 
                  className={`transition-colors duration-300 cursor-pointer ${
                    selectedRegion === 'eu' 
                      ? 'fill-indigo-500/20 stroke-indigo-500 stroke-2' 
                      : 'fill-slate-300/10 dark:fill-slate-700/10 stroke-slate-400/25 dark:stroke-slate-500/20 hover:fill-indigo-500/10 hover:stroke-indigo-500/50'
                ...
                {/* Africa */}
                <polygon 
                  points="400,200 480,200 510,290 470,360 410,290" 
                  className={`transition-colors duration-300 cursor-pointer ${
                    selectedRegion === 'af' 
                      ? 'fill-indigo-500/20 stroke-indigo-500 stroke-2' 
                      : 'fill-slate-300/10 dark:fill-slate-700/10 stroke-slate-400/25 dark:stroke-slate-500/20 hover:fill-indigo-500/10 hover:stroke-indigo-500/50'
                ...
                {/* Asia */}
                <polygon 
                  points="480,70 700,70 750,220 620,250 510,180" 
                  className={`transition-colors duration-300 cursor-pointer ${
                    selectedRegion === 'as' 
                      ? 'fill-indigo-500/20 stroke-indigo-500 stroke-2' 
                      : 'fill-slate-300/10 dark:fill-slate-700/10 stroke-slate-400/25 dark:stroke-slate-500/20 hover:fill-indigo-500/10 hover:stroke-indigo-500/50'
                ...
                {/* Oceania */}
                <polygon 
                  points="650,280 740,280 750,360 670,360" 
                  className={`transition-colors duration-300 cursor-pointer ${
                    selectedRegion === 'oc' 
                      ? 'fill-indigo-500/20 stroke-indigo-500 stroke-2' 
                      : 'fill-slate-300/10 dark:fill-slate-700/10 stroke-slate-400/25 dark:stroke-slate-500/20 hover:fill-indigo-500/10 hover:stroke-indigo-500/50'
                  }`}
                  onClick={() => setSelectedRegion('oc')}
                />
                {/* Arctic & Antarctica */}
                <polygon 
                  points="150,40 650,40 600,20 200,20" 
                  className={`transition-colors duration-300 cursor-pointer ${
                    selectedRegion === 'po' 
                      ? 'fill-indigo-500/20 stroke-indigo-500 stroke-2' 
                      : 'fill-slate-300/10 dark:fill-slate-700/10 stroke-slate-400/25 dark:stroke-slate-500/20 hover:fill-indigo-500/10 hover:stroke-indigo-500/50'
                  }`}
                  onClick={() => setSelectedRegion('po')}
                />
                <polygon 
                  points="150,410 650,410 600,430 200,430" 
                  className={`transition-colors duration-300 cursor-pointer ${
                    selectedRegion === 'po' 
                      ? 'fill-indigo-500/20 stroke-indigo-500 stroke-2' 
                      : 'fill-slate-300/10 dark:fill-slate-700/10 stroke-slate-400/25 dark:stroke-slate-500/20 hover:fill-indigo-500/10 hover:stroke-indigo-500/50'
                ...
                {/* Region Anchor Points (Interactive Glowing Buttons) */}
                {[
                  { id: 'na', x: 190, y: 150, label: t.regions.na },
                  { id: 'sa', x: 290, y: 310, label: t.regions.sa },
                  { id: 'eu', x: 420, y: 130, label: t.regions.eu },
                  { id: 'af', x: 460, y: 270, label: t.regions.af },
                  { id: 'as', x: 610, y: 160, label: t.regions.as },
                  { id: 'oc', x: 690, y: 320, label: t.regions.oc },
                  { id: 'po', x: 400, y: 30, label: t.regions.po }
                ].map((pt) => {
                  const isActive = selectedRegion === pt.id;
                  return (
                    <g 
                      key={pt.id} 
                    ...
                    >
                      ...
                      <circle 
                        ...
                        cy={pt.y} 
                        r={isActive ? "7" : "5"} 
                        fill={isActive ? "#6366f1" : "rgba(100, 116, 139, 0.8)"}
                        stroke="#ffffff"
                        strokeWidth="1.5"
                        className="transition-all duration-300"
                      />
                      {/* Floating Text Shadow Backdrop */}
                      <text
                        x={pt.x}
                        y={pt.y - 12}
                        textAnchor="middle"
                        className="text-[9px] font-black fill-slate-950 dark:fill-slate-900 stroke-white dark:stroke-slate-950 stroke-2 paint-order-stroke select-none"
                      >
                        {pt.label}
                      </text>
                      {/* Floating Text */}
              ...
              <div className="absolute bottom-2 right-2 bg-slate-950/70 text-[9px] text-slate-400 px-2 py-0.5 rounded backdrop-blur-sm">
                {t.selectPrompt}
              </div>
            </div>
            ...
            <div className="lg:col-span-5 h-full flex flex-col">
              {(() => {
                ...
                const getSeverityStyles = (risk: 'moderate' | 'high' | 'severe' | 'critical') => {
                  switch (risk) {
                    case 'moderate': 
                      return { 
                        bg: 'bg-yellow-100 dark:bg-yellow-950/40 text-yellow-800 dark:text-yellow-400 border-yellow-250 dark:border-yellow-900/50', 
                        label: t.risk.moderate,
                        bar: 'w-1/4 bg-yellow-500'
                      };
                    case 'high': 
                      return { 
                        bg: 'bg-orange-100 dark:bg-orange-950/40 text-orange-800 dark:text-orange-400 border-orange-250 dark:border-orange-900/50', 
                        label: t.risk.high,
                        bar: 'w-2/4 bg-orange-500'
                      };
                    case 'severe': 
                      return { 
                        bg: 'bg-red-100 dark:bg-red-950/40 text-red-800 dark:text-red-400 border-red-250 dark:border-red-900/50', 
                        label: t.risk.severe,
                        bar: 'w-3/4 bg-red-500'
                      };
                    case 'critical': 
                      return { 
                        bg: 'bg-rose-200 dark:bg-rose-950/60 text-rose-800 dark:text-rose-400 border-rose-300 dark:border-rose-900/60', 
                ...
                return (
                  <div className="flex flex-col h-full justify-between">
                    <div className="mb-4 flex items-center justify-between border-b border-slate-200 dark:border-slate-800/80 pb-2">
                    ...
                    <div className="flex flex-col gap-3 max-h-[350px] overflow-y-auto pr-1">
                      {sectors.map((sec) => {
                        const sev = getSeverityStyles(sec.info.risk);
                        return (
                          <div 
                            key={sec.id}
                            className="p-3 bg-white dark:bg-slate-950/40 border border-slate-150 dark:border-slate-850/50 rounded-xl flex flex-col gap-1.5 transition-all hover:shadow-sm"
                          >
                            <div className="flex items-center justify-between gap-2">
                              <div className="flex items-center gap-1.5 font-bold text-xs text-slate-700 dark:text-slate-350">
                                <span className="text-indigo-500 dark:text-indigo-400">{sec.icon}</span>
                                <span>{sec.title}</span>
                              </div>
                              <span className={`text-[9px] font-black uppercase px-2 py-0.5 rounded border ${sev.bg}`}>
                                {sev.label}
                              </span>
                            </div>
      ...
      {finalCaption && (
        <p className="text-center text-xs md:text-sm text-slate-500 dark:text-slate-400 italic mt-2 max-w-2xl px-4">
          {renderCaptionWithLinks(finalCaption, fallbackUrl || (hasValidImage ? src : undefined))}
        </p>
      )}
    </div>
  );
